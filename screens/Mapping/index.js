...
const MappingScreen = () => {
  ...
  // =============== Some work functions ===============
  const getMaps = () => {
    const config = { method: 'GET', url: `${API.url}/work/find_all_by_type_status/fr/Carte géographique/Pertinente`, headers: { 'X-localization': 'fr' } };
    ...
    axios(config)
      .then(res => {
        ...
        setMaps(mapsData);
        setTotalpages(mapsLastPage);
        setItemsPerPage(maps.length);
        setIsLoading(false);
          ...
          ListEmptyComponent={() => {
            return (
              <View style={[homeStyles.cardEmpty, { marginLeft: 0 }]}>
                ...
                <Text style={homeStyles.cardEmptyText}>{t('empty_list.description_maps')}</Text>
              </View>
            )
          }}
          renderItem={({ item }) => {
