...
const JournalScreen = () => {
  ...
  const getMags = () => {
    ...
    const url = `${API.url}/work/filter_by_categories_type_status/fr/Article/Pertinente?page=${currentPage}`;
    let mParams = {}
    let qs = require('qs');
    ...
    axios.post(url, qs.stringify(mParams), mHeaders).then(res => {
      ...
      setLastPage(magsLastPage);
      setMags([...mags, ...magsData]);
      setIsLoading(false);
      ...
      <SafeAreaView contentContainerStyle={{ flexGrow: 1 }}>
        <View style={[homeStyles.cardEmpty, { height: Dimensions.get('window').height - 70, marginLeft: 0, paddingLeft: 5 }]}>
          <FlatList
            ...
            ListFooterComponent={mags.length > 0 ? renderLoadMoreButton : null}
